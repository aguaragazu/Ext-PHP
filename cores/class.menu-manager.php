...
class menuManager extends msDB
{
    ...
    public function save(
        ...
        $conf = array();
        if ($menu_id) {
            $strSql = 'update menu set title=?,iconCls=?,module=?,handler=?,report=?,ajax=?,published=? where id=?';
            $args = array(
                    $menu_title,
                    $iconCls,
                    $module,
            ...
            $strSql = 'insert into menu (parent_id,title,iconcls,module,handler,ajax,report,published)';
            $strSql .= ' values(?,?,?,?,?,?,?,?);';
            $args = array(
                    $parent_id,
                    $menu_title,
     ...
     */
    public function getDetail($menu_id, $isMenu)
    {
        $msg['success'] = false;
        $query = "
     ...
     * @return success/failed Message
     */
    public function publish($dataList)
    {
     ...
     * @return success/failed message
     */
    public function remove($dataList)
    {
        $data = json_decode(stripslashes($dataList));
        foreach ($data as $row) {
     ...
     * @return json string
     */
    public function fileList($dir, $typefile = 'gif|png')
    {
        $images = array();
        $d = dir($dir);
        while ($name = $d->read()) {
    ...
    public function gridIcon($action, $REQUEST)
    {
        $grid = new grid(true);
    ...
    public function gridEvent($action, $REQUEST, $menu_id)
    {
        $grid = new grid(true);
    ...
    public function dropEventMenu($data)
    {
    ...
    public function dropRoleMenu($menu_id)
    {
    ...
    public function createRoleMenu($menu_id)
    {
    ...
    public function createEventMenu($data)
    {
    ...
    public function doDrop($node, $parent)
    {
    ...
    public function sortMenu($REQUEST, $parent_id, $update = 0)
    {
        $grid = new grid(true);
        $grid->setTable('menu');
